...
const EditCategory = () => {
  ...
  src={
    typeof productDetails.image === "string"
      ? `${config.API_URL}${productDetails.image}` // Backend image path
      : URL.createObjectURL(productDetails.image) // Local file object
  }
...
/> */}
  ...
  src={
    typeof productDetails.image === "string"
      ? productDetails.image // Backend image path
      : URL.createObjectURL(productDetails.image) // Local file object
  }
  alt={productDetails.image ? productDetails.image.name : " "}
  className="w-full h-32 object-cover rounded-3xl mb-2"
/>
          ...
          <div className="flex justify-between">
            ...
            <button
              ...
              className={`bg-transparent border-2 border-gray-300 dark:border-gray-700 text-black rounded-lg px-4 py-2 transition duration-300 ease-in-out ${loading ? 'opacity-50 cursor-not-allowed' : 'hover:bg-gray-400'
                } dark:text-white`}
              disabled={loading}
            >
