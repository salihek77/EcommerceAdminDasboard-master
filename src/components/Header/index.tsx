    ...
    <header className="sticky top-0 z-999 flex w-full bg-white drop-shadow-1 dark:bg-boxdark dark:drop-shadow-none">
      <div className="flex flex-grow items-center justify-between px-4 py-4 shadow-2 md:px-6 2xl:px-11">
        ...
        <div className="hidden sm:block">
          {/* <form action="https://formbold.com/s/unique_form_id" method="POST" onSubmit={(e) => e.preventDefault()} className="flex">
          ...
          {results.length > 0 && (
            <ul className="absolute left-0 bg-white drop-shadow-1 dark:bg-boxdark dark:drop-shadow-none right-0 mt-2  border border-gray-300 shadow-lg z-50 max-h-60 overflow-y-auto">
              {results.map((user) => (
                <li key={user._id} className="p-2 hover:bg-gray-100 cursor-pointer text-blue-500 hover:underline">
                  <Link to={`/profile/${user._id}`} className="block" onClick={handleResultClick}>
                    {user.productName} - {user.category}
                  </Link>
                </li>
              ))}
            </ul>
